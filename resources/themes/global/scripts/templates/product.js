...
(function(t){
    ...
    function onAddToCart(ev) {
        ...
        // toggle required for quantity inputs depending on which one is visible
        // on all the product templates there's one for desktop and one for mobile
        $form.find('.form-control[name=quantity]').each(function() {
        ...
        Givecloud.Cart().addProduct(data)
            .then(function(checkout){
                if (redirect_to) {
                    ...
                    theme.toast.success(theme.trans('scripts.templates.product.added_to_your_cart'));
                    theme.fire('theme.product.onAddToCart:success', [checkout]);
                }
            })
            .catch(function(err){
    ...
    function init($form) {
        ...
        function onVariantChange($element){
            theme.fire('theme.product:onVariantChange', [$element, $form]);

            var $productOptions   = $form.find('.product-options'),
                $pricingContainer = $productOptions.hasClass('product-options-list')
                    ? $form.find('.custom-control-input:checked + .custom-control-label .pricing-container')
                    : $form.find('.pricing-container'),
            ...
            $form.find('.pricing-container').empty();
            ...
            var data = {
                ...
                'preserve_amount': $element.data('preserve-amount'),
                'first_payment'  : $element.data('recurring-first-payment'),
                'pledge_campaign': null,
            };

            if (data.variant_id === null && $element.val()) {
                data.pledge_campaign = theme.toNumber(String($element.val()).replace(/^pledge_campaign:(\d+)$/,'$1'));
            }
            ...
            if (data.donation || data.pledge_campaign) {
                if (data.price_presets) {
                    var $presets = $('<div class="btn-group-toggle form-primary btn-group-checked d-inline-block price-presets" data-toggle="buttons" role="group" aria-labelledby="presets_heading"><label class="sr-only" id="presets_heading">' + theme.trans('scripts.templates.product.choose_an_amount') + '</label></div>').appendTo($pricingContainer);
                    $.each(data.price_presets, function(i, price){
                        if (price.toLowerCase() == 'other') {
                            ...
                            $option = $('<label class="btn btn-lg btn-outline-primary mb-1 mr-1"><input type="radio" name="preset_amount" value="' + price + '"> ' + theme.money(price, currency_code, { autoFractionDigits: true }) + '</small></label>').appendTo($presets);
                        }
                    });
                } else {
                    $option = $('<div class="form-group d-inline-block mb-1 mr-1 labelify bs-validate">' +
                            '<label for="p-variant-id-' + data.variant_id + '">' + theme.trans('scripts.templates.product.amount') + '</label>' +
                ...
                if ($otherAmount.length) {
                    $otherAmount.focus(function(){
                        ...
                        var decimal = (1.11).toLocaleString().replace(/1/g, '');
                        // prevent user from entering non-numeric characters
                        if (ev.key.length === 1 && ev.key !== decimal && /\D/.test(ev.key)) {
                            ev.preventDefault();
                        }
                        // prevent user from entering a second decimal delimiter
                        if (ev.key === decimal && ev.target.value.toString().indexOf('.') > -1) {
                            ev.preventDefault();
                        }
                    ...
                    if (data.minimum_price) {
                        $('<div class="minimum-price-notice w-100 mb-1 text-primary text-muted" data-minimum-price="' + theme.formatNumber(data.minimum_price, 2) + '">' + theme.trans('scripts.templates.product.minimum_donation', { currency_price: theme.money(data.minimum_price, currency_code) }) +
                        '</div>').appendTo($pricingContainer);
                    }
                }
                ...
                if (data.other_amount && $otherAmount.length) {
                    ...
                    if (default_amt) {
                        var $defaultAmountPreset = $pricingContainer.find('input[name=preset_amount][value="' + default_amt + '"]').first();
                ...
                if (!hide_pricing) {
                    product_meta += ((data.on_sale) ?
                        '<span class="line-thru text-muted font-weight-light">' + theme.money(data.price, currency_code) + '</span> ' + theme.money(data.sale_price, currency_code)
                        : theme.money(data.price, currency_code));
                    product_meta += ((bill_period_short) ? '/' + bill_period_short : '');
                }
                $amt.val(data.on_sale ? data.sale_price : data.price).change();
            }
            ...
            if (data.billing_period !== 'onetime' && ! data.pledge_campaign) {
    ...
    function checkValidity(form) {
        ...
        if (minPriceNotice.length) {
            ...
            var amount = theme.toNumber(minPriceNotice.parents('.product-options').find('input[name=amt]').val());
            var minimumPrice = theme.toNumber(minPriceNotice.data('minimum-price'));

...
import { CarouselButton, CarouselLink, DialogProps } from '@/aerosol'
import { Button, Column, Columns, DialogContent, DialogFooter, DialogHeader, Text, triggerToast } from '@/aerosol'
...
import styles from './InstantPopUpWidgetScreen.styles.scss'

interface Props extends Pick<DialogProps, 'onClose'> {
  formId: string
}

const InstantPopUpWidgetScreen: FC<Props> = ({ formId, onClose }) => {
  const { extraSmall } = useTailwindBreakpoints()

  const pixelCode = `<a href="#/fundraising/forms/${formId}">Donate</a>`
...
export { InstantPopUpWidgetScreen }
